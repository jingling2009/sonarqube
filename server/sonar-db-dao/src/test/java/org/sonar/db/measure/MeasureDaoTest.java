...
public class MeasureDaoTest {
  ...
  public void selectByComponentsAndMetrics() {
    ComponentDto project1 = db.components().insertProject();
    ComponentDto module = db.components().insertComponent(newModuleDto(project1));
    ComponentDto file1 = db.components().insertComponent(newFileDto(module).setUuid("C1"));
    ComponentDto file2 = db.components().insertComponent(newFileDto(module).setUuid("C2"));
    insertAnalysis(LAST_ANALYSIS_UUID, project1.uuid(), true);
    insertAnalysis(OTHER_ANALYSIS_UUID, project1.uuid(), false);
    ...
    assertThat(underTest.selectByComponentsAndMetrics(db.getSession(), singletonList(file1), singletonList(NCLOC_METRIC_ID))).extracting(MeasureDto::getData).containsOnly("M2");
    assertThat(underTest.selectByComponentsAndMetrics(db.getSession(), singletonList(file1), asList(NCLOC_METRIC_ID, COVERAGE_METRIC_ID))).extracting(MeasureDto::getData)
      .containsOnly("M2", "M3");

    // ncloc measures of components C1, C2
    assertThat(underTest.selectByComponentsAndMetrics(db.getSession(), asList(file1, file2), asList(NCLOC_METRIC_ID, COVERAGE_METRIC_ID))).extracting(MeasureDto::getData)
      .containsOnly("M2", "M3", "M6");

    // projects measures of last analysis
    assertThat(underTest.selectByComponentsAndMetrics(db.getSession(), singletonList(project1), singletonList(NCLOC_METRIC_ID))).extracting(MeasureDto::getData)
      .containsOnly("P1_M1");
    assertThat(underTest.selectByComponentsAndMetrics(db.getSession(), asList(project1, project2), asList(NCLOC_METRIC_ID, COVERAGE_METRIC_ID))).extracting(MeasureDto::getData)
